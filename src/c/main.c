...
static TextLayer *s_date_layer, *s_stardate_layer, *s_batterypercent_layer,
    *s_lifesupport_text_layer, *s_temperature_layer, *s_city_layer,
    *icon_weather_layer, *s_time_layer, *s_weekname_layer, *s_weeknum_layer,
    *s_weatherdescript_layer, *s_qt_layer;
static BitmapLayer *s_enterprise_layer;
static GBitmap *s_enterprise_bitmap;
static char api_key[50];
static char ReplacementWeatherMessage[21], userweatherprovider[7],
    WeekNumDisp[5], battery_text[] = "Pwr:100%", DateFormatter[] = "%m%d%y.";
static int s_step_count = 1, s_step_goal = 1, s_step_average = 1,
           prev_s_step_count = 1, UserStepGoal = 7500, s_battery_level,
           s_battery_charging;
static Layer *s_battery_layer, *s_lifesupport_layer, *s_lcars_layer,
    *s_bt_layer, *s_warp_layer;
static bool F_Tick = S_TRUE, PowerDisp = S_TRUE, UKDateFormat = S_FALSE,
            WeekdayNameDisp = S_TRUE, WeatherDescriptionDisp = S_FALSE,
            AnimationEnabled = S_TRUE, DisplaySeconds = S_FALSE,
            Watchface_Hibernate = S_FALSE, Watchface_Sleep = S_FALSE;
static int warpsequence = 0, text_color_value = 0;
static int loweracrossline = 90, upperacrossline = 50, textleft = 25,
           watchwidth = 168, /*slept=0,*/ TIMER_IDLE_INTERVAL = 46,
           TIMER_INTERVAL_MS = 500, Current_Min = 61, Current_Hour = 24,
           Hibernate_Min = 61;
static PropertyAnimation *image_property_animation;  // 1800000
static bool WeatherSetupStatusKey = S_FALSE,
            WeatherSetupStatusProvider = S_FALSE,
            WeatherReadyRecieved = S_FALSE, HibernateEnable = S_TRUE,
            SleepEnable = S_TRUE;
GColor text_color;
static EventHandle s_health_event_handle = NULL,
                   s_tick_timer_event_handle =
                       NULL;  //, s_idle_timer_event_handle;
...
static void read_persist() {
   ...
   if (persist_exists(MESSAGE_KEY_WEATHERREPLACEMENT)) {
      persist_read_string(MESSAGE_KEY_WEATHERREPLACEMENT,
                          ReplacementWeatherMessage,
                          sizeof(ReplacementWeatherMessage));
   }
   ...
   if (persist_exists(MESSAGE_KEY_WEATHERDESCRIPTION)) {
      WeatherDescriptionDisp =
          persist_read_bool(MESSAGE_KEY_WEATHERDESCRIPTION);
   }
   ...
   if (persist_exists(MESSAGE_KEY_WeatherProvide)) {
      persist_read_string(MESSAGE_KEY_WeatherProvide, userweatherprovider,
                          sizeof(userweatherprovider));
   }
   if (persist_exists(MESSAGE_KEY_WEEKNUMFORMAT)) {
      persist_read_string(MESSAGE_KEY_WEEKNUMFORMAT, WeekNumDisp,
                          sizeof(WeekNumDisp));
   }
}
...
static void store_persist() {
   ...
   persist_write_string(MESSAGE_KEY_WEATHERREPLACEMENT,
                        ReplacementWeatherMessage);
   ...
   if (text_color_value > 0) {
      persist_write_int(MESSAGE_KEY_TEXTCOLOR, text_color_value);
   }
}
   ...
   switch (average) {
      case 0: {
         ...
         if (mask & HealthServiceAccessibilityMaskAvailable) {
            // APP_LOG(APP_LOG_LEVEL_INFO, "Metric: %d",
      ...
      case 1: {
         ...
         HealthServiceAccessibilityMask mask =
             health_service_metric_averaged_accessible(
                 metric, start, end, HealthServiceTimeScopeWeekly);
         if (mask & HealthServiceAccessibilityMaskAvailable) {
            // Average is available, read it
            output = (int)health_service_sum_averaged(
                metric, start, end, HealthServiceTimeScopeWeekly);
         }
         //    output = (int)health_service_sum_averaged(metric, start, end,
      ...
      case 2: {
         HealthServiceAccessibilityMask mask =
             health_service_metric_averaged_accessible(
                 metric, start, start + SECONDS_PER_DAY,
      ...
      default: { APP_LOG(APP_LOG_LEVEL_ERROR, "Health data unavailable!"); }
   }
   return output;
}
...
static void lifesupport_update_proc(Layer *layer, GContext *ctx) {
   ...
   if (goalwidth > bounds.size.w) {
      goalwidth = bounds.size.w;
   }
   int avgwidth = ((s_step_count * bounds.size.w) / s_step_average);
   if (avgwidth > bounds.size.w) {
      avgwidth = bounds.size.w;
   }
   ...
   graphics_fill_rect(ctx,
                      GRect(0, bounds.size.h / 2, goalwidth, bounds.size.h / 2),
                      0, GCornerNone);
   graphics_context_set_fill_color(ctx, GColorMidnightGreen);
   graphics_fill_rect(ctx, GRect(0, 0, avgwidth, bounds.size.h / 2), 0,
                      GCornerNone);
}

static void lcars_block(Layer *layer, GContext *ctx) {
   // LCARS COLORS GColorVividViolet GColorIcterine GColorLavenderIndigo
   // GColorPictonBlue GColorRajah GColorBlue GColorChromeYellow
   // GColorSunsetOrange
   //              dark purple       light yellow    light purple        light
   //              blue        tan
   GRect bounds = layer_get_bounds(layer);
   ...
   graphics_fill_rect(
       ctx, GRect(3, loweracrossline, (bounds.size.w / 4) - 3, blockheight), 15,
       GCornerTopLeft);  // 3,90,33,15,15R -> 3,120,47,21,
   // graphics_fill_rect(ctx, GRect(3,90,33,15), 15, GCornerTopLeft);
   graphics_context_set_fill_color(ctx, GColorBlack);
   graphics_fill_rect(
       ctx, GRect(blockwidth, thinblockheight + loweracrossline, 17, 15), 10,
       GCornerTopLeft);  // 24,94,12,11,10R ->33,126,17,15
   // graphics_fill_rect(ctx, GRect(24,94,12,11), 10, GCornerTopLeft);
   // lower elbo +1
   graphics_context_set_fill_color(ctx, GColorIcterine);
   graphics_fill_rect(
       ctx, GRect(basewidth, loweracrossline, blockwidth, thinblockheight), 0,
       GCornerNone);
   // graphics_fill_rect(ctx, GRect(38,90,21,4), 0, GCornerNone);
   // //38,90,24,4>52,120,33,6
   // lower elbo +2
   graphics_context_set_fill_color(ctx, GColorVividViolet);
   graphics_fill_rect(ctx, GRect(basewidth + spacing + blockwidth,
                                 loweracrossline, blockwidth, thinblockheight),
                      0, GCornerNone);
   // graphics_fill_rect(ctx, GRect(62,90,21,4), 0, GCornerNone);
   // //64,90,24,4->88,120,33,6
   // lower elbo +3
   graphics_context_set_fill_color(ctx, GColorBlue);
   graphics_fill_rect(ctx, GRect(basewidth + 2 * (spacing + blockwidth),
                                 loweracrossline, blockwidth, thinblockheight),
                      0, GCornerNone);
   // graphics_fill_rect(ctx, GRect(86,90,21,4), 0, GCornerNone);
   // //90,90,24,4->124,120,33,6
   // lower elbo +4
   graphics_context_set_fill_color(ctx, GColorPictonBlue);
   graphics_fill_rect(ctx,
                      GRect(basewidth + 3 * (spacing + blockwidth),
                            loweracrossline, blockwidth + 7, thinblockheight),
                      0, GCornerNone);
   // graphics_fill_rect(ctx, GRect(110,90,34,4), 0, GCornerNone);
   // //116,90,28,4 ->160,120,40,6  //accounts for the display having a slightly
   // longer last block
   // lower elbo -1
   graphics_context_set_fill_color(ctx, GColorChromeYellow);
   graphics_fill_rect(ctx, GRect(3, loweracrossline + blockheight + spacing,
                                 blockwidth - 3, blockheight),
                      0, GCornerNone);
   //  graphics_fill_rect(ctx, GRect(3,107,21,15), 0, GCornerNone);
   //  //->3,144,30,21
   // lower elbo -2
   graphics_fill_rect(ctx,
                      GRect(3, loweracrossline + 2 * (blockheight + spacing),
                            blockwidth - 3, blockheight),
                      0, GCornerNone);
   //  graphics_fill_rect(ctx, GRect(3,124,21,15), 0, GCornerNone);
   //  //->3,168,30,21
   // lower elbo -3
   if (connected) {
   ...
   graphics_fill_rect(ctx,
                      GRect(3, loweracrossline + 3 * (blockheight + spacing),
                            blockwidth - 3, 36),
                      0, GCornerNone);
   //  graphics_fill_rect(ctx, GRect(3,141,21,27), 0, GCornerNone);
   //  //->3,192,30,36 36 because the last block is higher
   // upper elbo
   graphics_context_set_fill_color(ctx, GColorPictonBlue);
   graphics_fill_rect(ctx, GRect(3, upperacrossline - blockheight,
                                 blockwidth * 3, blockheight + blockheight / 2),
                      15, GCornerBottomLeft);
   graphics_context_set_fill_color(ctx, GColorBlack);
   graphics_fill_rect(
       ctx, GRect(blockwidth, upperacrossline - blockheight, blockwidth * 2 + 3,
                  blockheight + blockheight / 2 - thinblockheight),
       10, GCornerBottomLeft);
   // upper elbo +1
   graphics_context_set_fill_color(ctx, GColorChromeYellow);
   graphics_fill_rect(ctx,
                      GRect(blockwidth * 3 + 3 + spacing,
                            upperacrossline + blockheight / 2 - thinblockheight,
                            blockwidth, thinblockheight),
                      0, GCornerNone);
   //  graphics_fill_rect(ctx, GRect(77,53,21,4), 0, GCornerNone);
   // upper elbo +2
   graphics_context_set_fill_color(ctx, GColorSunsetOrange);
   graphics_fill_rect(ctx,
                      GRect(blockwidth * 4 + 3 + spacing * 2,
                            upperacrossline + blockheight / 2 - thinblockheight,
                            blockwidth * 2, thinblockheight),
                      0, GCornerNone);
   //  graphics_fill_rect(ctx, GRect(110,53,44,4), 0, GCornerNone);
   // upper elbo -1
   graphics_context_set_fill_color(ctx, GColorLavenderIndigo);
   graphics_fill_rect(ctx, GRect(3, upperacrossline - 2 * blockheight - spacing,
                                 blockwidth - 3, blockheight),
                      0, GCornerNone);
   //  graphics_fill_rect(ctx, GRect(3,18,21,15), 0, GCornerNone);
   // upper elbo -2
   graphics_context_set_fill_color(ctx, GColorPictonBlue);
   graphics_fill_rect(ctx, GRect(3, 0, blockwidth - 3,
                                 upperacrossline - 2 * (blockheight + spacing)),
                      0, GCornerNone);
...
static void weather_callback(GenericWeatherInfo *info,
   ...
   if ((strlen(ReplacementWeatherMessage) == 0) &&
       (connection_service_peek_pebble_app_connection() == S_TRUE)) {
      ...
      switch (status) {
         case GenericWeatherStatusAvailable: {
            ...
            //      snprintf(s_buffer, sizeof(s_buffer),"Temperature (K/C/F):
            //      %d/%d/%d\n\nName:\n%s\n\nDescription:\n%s",info->temp_k,
            //      info->temp_c, info->temp_f, info->name, info->description);
            if (F_Tick) {
            ...
            if (WeatherDescriptionDisp) {
               snprintf(s_weather_description, sizeof(s_weather_description),
                        "%s", info->description);
               text_layer_set_text(s_weatherdescript_layer,
                                   s_weather_description);
            } else {
               text_layer_set_text(s_weatherdescript_layer, " ");
            }
         ...
         case GenericWeatherStatusBluetoothDisconnected:
            ...
            snprintf(StatusTimestamp, sizeof(StatusTimestamp), "NoBT@%02d:%02d",
         ...
         case GenericWeatherStatusFailed:
            ...
            snprintf(StatusTimestamp, sizeof(StatusTimestamp), "Fail@%02d:%02d",
                     Current_Hour, Current_Min);
            text_layer_set_text(s_weatherdescript_layer, StatusTimestamp);
            break;
         ...
         case GenericWeatherStatusLocationUnavailable:
            ...
            snprintf(StatusTimestamp, sizeof(StatusTimestamp),
                     "NoLoc@%02d:%02d", Current_Hour, Current_Min);
            text_layer_set_text(s_weatherdescript_layer, StatusTimestamp);
            break;
      }
   }
...
static void bt_update_proc(Layer *layer, GContext *ctx) {
   ...
   if (connected) {
      ...
      text_layer_set_text(
          s_temperature_layer,
          "…");  // this is the voyager com badge icon for the custom font
      ...
      if (WeatherDescriptionDisp) {
         static char StatusTimestamp[18];
         snprintf(StatusTimestamp, sizeof(StatusTimestamp), "NoBT@%d:%d",
                  Current_Hour, Current_Min);
         text_layer_set_text(s_weatherdescript_layer, StatusTimestamp);
      }
   }
...
static void warp_update_proc(Layer *layer, GContext *ctx) {
   ...
   if (warpsequence > 0) {
      graphics_context_set_fill_color(ctx, GColorPictonBlue);
      graphics_fill_rect(
          ctx, GRect(0, 6 - warpsequence, bounds.size.w, 2 * warpsequence + 1),
          2, GCornersAll);
   }
}

static void update_battery() { battery_callback(battery_state_service_peek()); }
...
static void trigger_animation() {
   if ((warpsequence > 0) && (warpsequence < 8)) {
      ...
      // Set start and end
      GRect from_frame =
          layer_get_frame(bitmap_layer_get_layer(s_enterprise_layer));
      GRect to_frame = GRect(watchwidth, upperacrossline + 8, watchwidth, 32);

      // Create the animation
      image_property_animation = property_animation_create_layer_frame(
          bitmap_layer_get_layer(s_enterprise_layer), &from_frame, &to_frame);

      // Configure the animation, these are optional
      animation_set_duration((Animation *)image_property_animation,
                             TIMER_INTERVAL_MS);  // milliseconds
      animation_set_delay((Animation *)image_property_animation,
                          10);  // milliseconds
      animation_set_curve((Animation *)image_property_animation,
...
static void window_load(Window *window) {
   ...
   s_bt_layer =
       layer_create(GRect((bounds.size.w / 14) - 7,
                          bounds.size.h - (bounds.size.h / 12 + 10), 21, 23));
   //  s_bt_layer = layer_create(GRect(3,144,21,23));
   layer_set_update_proc(s_bt_layer, bt_update_proc);
   layer_add_child(window_layer, s_bt_layer);

   s_qt_layer = text_layer_create(
       GRect(3, bounds.size.h - (bounds.size.h / 12 + 10) - 22,
             bounds.size.w / 6 - 3, 19));
   ...
   // Create GFonts
   s_date_font =
       fonts_load_custom_font(resource_get_handle(RESOURCE_ID_LCARS_32));
   s_time_font =
       fonts_load_custom_font(resource_get_handle(RESOURCE_ID_LCARS_58));
   s_weather_icon_font =
       fonts_load_custom_font(resource_get_handle(RESOURCE_ID_WEATHERICON_38));

   // Create the TextLayer with specific bounds
   s_time_layer =
       text_layer_create(GRect(0, loweracrossline - 7, bounds.size.w, 80));
   ...
   s_enterprise_layer = bitmap_layer_create(GRect(
       0, (upperacrossline + loweracrossline) / 2 - 12, bounds.size.w, 32));
   ...
   s_date_layer = text_layer_create(
       GRect(textleft, 15, bounds.size.w, 40));  // the bounds width is wider
                                                 // than the display because
                                                 // certain numbers don't fit on
                                                 // the screen by a pixel or two
   ...
   // Create the TextLayer with specific bounds
   icon_weather_layer =
       text_layer_create(GRect(textleft, loweracrossline - 5, 40, 40));
   ...
   s_temperature_layer = text_layer_create(
       GRect(textleft, (bounds.size.h / 6 + 2) + loweracrossline, 40, 32));
   ...
   s_city_layer = text_layer_create(
       GRect(textleft, 2 * (bounds.size.h / 6 + 2) + loweracrossline,
             bounds.size.w - textleft, 32));
   ...
   //  } else {
   //    text_layer_set_font(s_city_layer,
   ...
   s_weatherdescript_layer = text_layer_create(
       GRect(bounds.size.w / 2 + 2, (upperacrossline + loweracrossline) / 2 + 3,
             bounds.size.w, 32));  // the width is intentionally wrong to let
                                   // the text wrap off the screen
   // s_weatherdescript_layer = text_layer_create(GRect(74, 74, bounds.size.w,
   // 32));
   ...
   //  if (bounds.size.w==144) {
   text_layer_set_font(s_weatherdescript_layer,
   ...
   // Create the TextLayer with specific bounds
   s_batterypercent_layer = text_layer_create(GRect(
       ((bounds.size.w * 13) / 28) - 64,
       upperacrossline + (4 * bounds.size.h / 100), bounds.size.w / 2, 15));
   // s_batterypercent_layer = text_layer_create(GRect(2, 54, bounds.size.w,
   // 15));
   // Apply to TextLayer
   text_layer_set_font(s_batterypercent_layer,
                       fonts_get_system_font(FONT_KEY_GOTHIC_14));
   // Improve the layout to be more like a watchface
   ...
   s_battery_layer = layer_create(
       GRect((bounds.size.w / 2) - 68, ((bounds.size.h * 25) / 60) + 2, 34, 5));
   ...
   s_lifesupport_layer =
       layer_create(GRect(bounds.size.w - 76, bounds.size.h / 2 + 59, 75,
                          ((bounds.size.h * 7) / 30) - 25));
   ...
   // Create the TextLayer with specific bounds
   s_lifesupport_text_layer = text_layer_create(
       GRect(bounds.size.w - 76, ((bounds.size.h * 7) / 12) + 42, 75, 19));
   // s_lifesupport_text_layer = text_layer_create(GRect(75, 140, 92, 15));
   // Apply to TextLayer
   //  if (bounds.size.w==144) {
   text_layer_set_font(s_lifesupport_text_layer,
                       fonts_get_system_font(FONT_KEY_GOTHIC_14));
   //  } else {
   //    text_layer_set_font(s_lifesupport_text_layer,
   //    fonts_get_system_font(FONT_KEY_GOTHIC_18));
   ...
   text_layer_set_text_alignment(s_lifesupport_text_layer,
                                 GTextAlignmentCenter);
   text_layer_set_text(s_lifesupport_text_layer, "Life Support");
   // Add it as a child layer to the Window's root layer
   layer_add_child(window_layer,
                   text_layer_get_layer(s_lifesupport_text_layer));
   ...
   // Apply to TextLayer
   //  if (bounds.size.w==144) {
   text_layer_set_font(s_weekname_layer,
                       fonts_get_system_font(FONT_KEY_GOTHIC_14));
   //  } else {
   //    text_layer_set_font(s_weekname_layer,
   //    fonts_get_system_font(FONT_KEY_GOTHIC_18));
   ...
   // Create the TextLayer with specific bounds
   s_weeknum_layer = text_layer_create(
       GRect(3, ((bounds.size.h * 7) / 60) - 3, bounds.size.w / 6 - 3, 19));
   // s_weeknum_layer = text_layer_create(GRect(3,16,21,15));
   // Apply to TextLayer
   //  if (bounds.size.w==144) {
   text_layer_set_font(s_weeknum_layer,
                       fonts_get_system_font(FONT_KEY_GOTHIC_14));
   //  } else {
   //    text_layer_set_font(s_weeknum_layer,
   //    fonts_get_system_font(FONT_KEY_GOTHIC_18));
   ...
   // Create the TextLayer with specific bounds
   s_stardate_layer =
       text_layer_create(GRect(textleft, 00, bounds.size.w - textleft, 19));
   // s_stardate_layer = text_layer_create(GRect(textleft, 00, bounds.size.w,
   // 15));
   // Apply to TextLayer
   //  if (bounds.size.w==144) {
   text_layer_set_font(s_stardate_layer,
   ...
   s_warp_layer = layer_create(GRect((bounds.size.w / 2) - 68,
                                     ((bounds.size.h * 25) / 60) - 2, 33, 15));
...
static void update_time() {
   ...
   strftime(s_buffer, sizeof(s_buffer), clock_is_24h_style() ? "%H%M" : "%l%M",
            tick_time);
   ...
   if (DisplaySeconds) {
      ...
      snprintf(date_step_buffer, sizeof(date_step_buffer), "%s%s", date_buffer,
               s_seconds_buffer);
   } else {
      // pull the current step count appending to the date buffer
      snprintf(date_step_buffer, sizeof(date_step_buffer), "%s%d", date_buffer,
               s_step_count);
   }
   // Show the stardate
...
static void update_steps() {
   ...
   if (prev_s_step_count < 2) {
      prev_s_step_count = s_step_count;
   }
   s_step_average = get_health(HealthMetricStepCount, 1);
   s_step_goal = get_health(HealthMetricStepCount, 2);
   if ((s_step_goal == 0) || (s_step_average == 0)) {  // this comes up when the
                                                       // watch has less than a
                                                       // week of health data
      time_t temp = time(NULL);
      struct tm *tick_time = localtime(&temp);
      s_step_goal = UserStepGoal;  // just use the user's step goal
      s_step_average = (((tick_time->tm_hour + 1) * s_step_goal) /
                        24);  // calculate average by the time of day
   }

   //  UserStepGoal = 1120;
   if ((prev_s_step_count < UserStepGoal) && (s_step_count >= UserStepGoal)) {
      static const uint32_t const segments[] = {200, 200, 50,  50, 50,
                                                50,  100, 200, 400};
      VibePattern pat = {
...
static void watchface_refresh() {
   ...
   if ((strlen(ReplacementWeatherMessage) == 0) &&
      ...
      text_layer_set_text(s_temperature_layer,
                          "…");  // place the star trek comm badge
   }
}
...
static void accel_tap_handler(AccelAxisType axis, int32_t direction) {
   ...
   if (HibernateEnable) {
      ...
      Hibernate_Min = (Current_Min + TIMER_IDLE_INTERVAL) %
                      60;  // I tried using a timer above but the interval is
                           // not set correctly on first run and the user has to
                           // relaunch the watchface.  Instead I am building my
                           // own timer
   } else {
      Hibernate_Min = 61;  // this will never be the current minute so this
...
static void tick_handler(struct tm *tick_time, TimeUnits units_changed) {
   if (!Watchface_Hibernate &&
       !Watchface_Sleep) {  // if we are no sleeping or hibernating
      ...
      if ((tick_time->tm_min % 30 == 0) && (tick_time->tm_sec == 0)) {
         ...
         if (PowerDisp) {
            snprintf(battery_text, sizeof(battery_text), "Pwr:%d%%",
                     s_battery_level);
            text_layer_set_text(s_batterypercent_layer, battery_text);
         }
      }
      if ((tick_time->tm_min == Hibernate_Min) && HibernateEnable &&
          !Watchface_Sleep) {
   ...
   /*  static char s_buffer[8];
  snprintf(s_buffer,sizeof(s_buffer),"%d",Hibernate_Min);
   ...
   switch (event) {
      ...
      case HealthEventSleepUpdate: {
         if (SleepEnable) {
            ...
            //        if (sleeping &&
            //        ((s_tick_timer_event_handle!=NULL)||Watchface_Hibernate))
            //        {
            if (sleeping && !Watchface_Sleep) {
               ...
               Watchface_Hibernate = S_FALSE;  // if the user falls asleep while
                                               // in hibernation then reset to
                                               // wake when the user wakes
               /*          static char s_buffer[8];
          time_t temp = time(NULL);
          struct tm *tick_time = localtime(&temp);
...
*/ accel_tap_service_subscribe(accel_tap_handler);
               ...
               accel_tap_service_unsubscribe();  // do this here because having
                                                 // a Null tick_timer coorelates
                                                 // with subscribing to tap
                                                 // service
               Watchface_Sleep = S_FALSE;
               Watchface_Hibernate = S_FALSE;
               watchface_refresh();
      ...
      case HealthEventMovementUpdate:
         if (Watchface_Hibernate || Watchface_Sleep) {
            ...
            Watchface_Hibernate = S_FALSE;
            watchface_refresh();
            accel_tap_service_unsubscribe();  // do this here because having a
                                              // Null tick_timer coorelates with
                                              // subscribing to tap service
         }
         if (HibernateEnable && !Watchface_Hibernate) {
            Hibernate_Min = (Current_Min + TIMER_IDLE_INTERVAL) % 60;
         }
         // app_timer_reschedule(s_idle_timer_event_handle,
         // TIMER_IDLE_INTERVAL_MS);
         break;
...
/*static void idle_scheduler(){
  if (HibernateEnable) {
    ...
    Watchface_Hibernate = S_TRUE;  //signifies that the watchface went idle,
used so that a healthevent does not wake up the watch
    accel_tap_service_subscribe(accel_tap_handler);
    //prv_health_event_handler(HealthEventSleepUpdate, NULL);
    s_idle_timer_event_handle =
app_timer_register(TIMER_IDLE_INTERVAL_MS*16,idle_scheduler,NULL);  //the only
time the idle handeler is used is when this function is called, so create a new
one that can be rescheduled by tap handeler or something else.
  }
}*/

static void WeatherInitDeinit() {
   if ((strlen(ReplacementWeatherMessage) == 0) &&
       ((strcmp(userweatherprovide, "YahooW") == 0) || (strlen(api_key) > 0))) {
      // APP_LOG(APP_LOG_LEVEL_DEBUG, "WeatherInit: -%s-", userweatherprovider);
      if (WeatherSetupStatusProvider == S_FALSE) {
         generic_weather_init();
      }
      if (strlen(api_key) > 0) generic_weather_set_api_key(api_key);
      ...
      if (strcmp(userweatherprovider, "YahooW") == 0) {
         ...
         APP_LOG(APP_LOG_LEVEL_DEBUG, "UNKNOWN PROVIDER: -%s-",
                 userweatherprovider);
      }
...
static void inbox_received_callback(DictionaryIterator *iterator,
   ...
   if (data) {
      ...
      // register to recieve significant updates of which we are looking for
      // sleep updates
      ...
      if (SleepEnable || HibernateEnable) {
         s_health_event_handle = events_health_service_events_subscribe(
             prv_health_event_handler, NULL);
      }

      // register to detect when the watch goes idle
      /*    if (HibernateEnable) {
            s_idle_timer_event_handle =
         app_timer_register(TIMER_IDLE_INTERVAL_MS,idle_scheduler,NULL);
          } else {
   ...
   if (data) {  // all setting are set each time the user saves settings.  Since
                // this is the last one refresh all displays so their new
                // settings.
...
static void init() {
   // I shouldn't be doing this here and it should be in read_persist, but the
   // function takes a while to complete and I need this setting for window_load
   text_color = persist_exists(MESSAGE_KEY_TEXTCOLOR)
                    ? GColorFromHEX(persist_read_int(MESSAGE_KEY_TEXTCOLOR))
                    : GColorChromeYellow;
   ...
   // register to recieve significant updates of which we are looking for sleep
   // updates
   if (SleepEnable || HibernateEnable) {
      s_health_event_handle = events_health_service_events_subscribe(
          prv_health_event_handler, NULL);
   ...
   // register to detect when the watch goes idle
   // if (HibernateEnable) {s_idle_timer_event_handle =
   // app_timer_register(TIMER_IDLE_INTERVAL_MS,idle_scheduler,NULL);}
   if (HibernateEnable) {
      Hibernate_Min = (Current_Min + TIMER_IDLE_INTERVAL) % 60;
   }
   ...
   events_connection_service_subscribe((ConnectionHandlers){
...
static void deinit() {
   if (WeatherSetupStatusProvider == S_TRUE) {
      generic_weather_deinit();
   }
   store_persist();
}
